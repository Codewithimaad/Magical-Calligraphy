...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setMessage({ text: '', type: '' });
        setErrors({ fullName: '', email: '', whatsapp: '', paymentScreenshot: '' });

        let isValid = true;
        const newErrors = { fullName: '', email: '', whatsapp: '', paymentScreenshot: '' };

        if (!e.target.fullName.value.trim()) { newErrors.fullName = 'Full name is required'; isValid = false; }
        if (!e.target.email.value.trim()) { newErrors.email = 'Email is required'; isValid = false; } 
        else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.target.email.value)) { newErrors.email = 'Please enter a valid email'; isValid = false; }
        if (!e.target.whatsapp.value.trim()) { newErrors.whatsapp = 'Whatsapp number is required'; isValid = false; }
        if (!file) { newErrors.paymentScreenshot = 'Please upload your payment screenshot'; isValid = false; }
        ...
        try {
            setLoading(true);
            const response = await axios.post(`${backendUrl}/api/users/register`, formData, { withCredentials: true });
    ...
    const paymentCards = [
        { name: 'Easypaisa', logo: easypaisaLogo, account: 'Haroon Alam Khan', number: '0332-5275117', bgGradient: 'bg-gray-100', shadowColor: 'shadow-gray-300', copyContent: '0332-5275117' },
        { name: 'Nayapay', logo: nayapayLogo, account: 'Haroon Alam Khan', number: '0330-9082834', bgGradient: 'bg-blue-100', shadowColor: 'shadow-gray-400', copyContent: '0330-9082834' },
        { name: 'Sadapay', logo: sadapayLogo, account: 'Haroon Alam Khan', number: '0332-5275117', iban: 'PK51SADA0000003325275117', bgGradient: 'bg-green-100', shadowColor: 'shadow-gray-300', copyContent: 'PK51SADA0000003325275117' }
    ];
    ...
    return (
        <div className="min-h-screen relative overflow-hidden mt-10">
            ...
            <div className="absolute inset-0">
                {[...Array(20)].map((_, i) => (
                    <div key={i} className="absolute w-2 h-2 bg-white/10 rounded-full animate-float"
                        style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, animationDelay: `${Math.random() * 5}s`, animationDuration: `${3 + Math.random() * 4}s` }}></div>
                ))}
            </div>
                        ...
                        <div className="p-2 md:p-12 space-y-12">
                            <div className="backdrop-blur-sm bg-gradient-to-r from-cyan-500/10 to-purple-500/10 rounded-2xl p-6 md:p-8 border border-cyan-300/20 transform-gpu transition-all duration-500 hover:scale-[1.02]">
                                ...
                                <ol className="list-decimal pl-8 space-y-3 text-gray-700">
                                    <li>Complete your registration form below</li>
                                    <li>Send payment to any account below</li>
                                    <li>Take a screenshot of payment confirmation</li>
                                    <li>Attach your Payment Screenshot Below in the form.</li>
                                </ol>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {paymentCards.map((card, index) => (
                                    <div key={card.name} className="group relative transform-gpu transition-all duration-700 hover:scale-105 hover:-translate-y-4" style={{ perspective: '1000px' }}>
                                            ...
                                            <div className="relative space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center justify-between bg-white/10 rounded-lg p-3 backdrop-blur-sm">
                                                        ...
                                                        <button onClick={() => copyToClipboard(card.copyContent, `${index}-number`)}
                                                            className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-all duration-300 transform hover:scale-110" title="Copy to clipboard">
                                                ...
                                                {card.iban && (
                                                    <div>
                                                        ...
                                                        <div className="flex items-center justify-between bg-white/10 rounded-lg p-3 backdrop-blur-sm">
                                                            ...
                                                            <button onClick={() => copyToClipboard(card.iban, `${index}-iban`)}
                                                                className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-all duration-300 transform hover:scale-110 ml-2" title="Copy IBAN">
                            ...
                            <form onSubmit={handleSubmit}
                                className="backdrop-blur-sm bg-white/40 rounded-2xl shadow-2xl p-6 md:p-8 border border-white/20 transform-gpu transition-all duration-500 hover:bg-white/50 mt-12">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="md:col-span-2 space-y-2">
                                        ...
                                        <div className="relative">
                                            <input type="file" name="paymentScreenshot" accept="image/png, image/jpeg, image/jpg"
                                            ...
                                            {errors.paymentScreenshot && <p className="mt-1 text-sm text-red-600">{errors.paymentScreenshot}</p>}
                                        </div>
                                    </div>
